...
var t=$("#card-body").get(0),e=$('.toCanvas');e.click(function(){var i=document.querySelector('#card-body img');i.complete?(generateCanvas()):i.onload=generateCanvas});
function generateCanvas(){html2canvas(t,{allowTaint:!1,useCORS:!0,scale:2}).then(function(c){var w=c.width,h=c.height,o=document.createElement('canvas');o.width=w,o.height=h;var f=o.getContext('2d');f.drawImage(c,0,0,w,h),f.font='italic bold 12px Arial',f.fillStyle='rgba(217,214,214,0.1)',f.textAlign='center',f.textBaseline='middle',f.rotate(-5*Math.PI/180);var n=document.title,l=window.location.href,a=n+' - '+l,s=f.measureText(a).width,r=12;var d=s+100,u=r+100;for(var p=0;p<w;p+=d)for(var m=0;m<h;m+=u)f.fillText(a,p+(d-s)/2,m+(u-r)/2);f.setTransform(1,0,0,1,0,0),e.after(o),f.imageSmoothingEnabled=!0,f.mozImageSmoothingEnabled=!0,f.webkitImageSmoothingEnabled=!0,f.oImageSmoothingEnabled=!0,f.msImageSmoothingEnabled=!0})}
const langToggle = document.getElementById('langToggle'); const langMenu = document.getElementById('langMenu'); langToggle.onclick = function(event) { event.preventDefault(); langMenu.classList.toggle('show'); }; window.onclick = function(event) { if (!langMenu.contains(event.target) && !langToggle.contains(event.target)) { langMenu.classList.remove('show'); } };
