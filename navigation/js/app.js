...
var test = $("#card-body").get(0);

// 点击转成canvas
$('.toCanvas').click(function(e) {
    var img = document.querySelector('#card-body img');
    if (img.complete) {
        generateCanvas();
    } else {
        img.onload = function() {
    ...
    function generateCanvas() {
        html2canvas(test, {
            allowTaint: false,
            useCORS: true,
            scale: 2, // 使用两倍的分辨率来提高质量
        }).then(function(canvas) {
            var canvasWidth = canvas.width;
            var canvasHeight = canvas.height;

            var watermarkedCanvas = document.createElement('canvas');
            watermarkedCanvas.width = canvasWidth;
            watermarkedCanvas.height = canvasHeight;

            var ctx = watermarkedCanvas.getContext('2d');

            ctx.drawImage(canvas, 0, 0, canvasWidth, canvasHeight);

            ctx.font = 'italic bold 12px Arial';
            ctx.fillStyle = 'rgba(217, 214, 214, 0.1)';
            ...
            var watermarkText = websiteName + ' - ' + websiteUrl; // 水印文字
            var watermarkWidth = ctx.measureText(watermarkText).width;
            var watermarkHeight = 12;

            var intervalX = watermarkWidth + 100;
            var intervalY = watermarkHeight + 100;

            for (var x = 0; x < canvasWidth; x += intervalX) {
                for (var y = 0; y < canvasHeight; y += intervalY) {
                    ctx.fillText(watermarkText, x + (intervalX - watermarkWidth) / 2, y + (intervalY - watermarkHeight) / 2);
                }
            }

            ctx.setTransform(1, 0, 0, 1, 0, 0); // 重置变换矩阵

            $('.toCanvas').after(watermarkedCanvas);
